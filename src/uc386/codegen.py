...
from uc_core.codegen_helpers import (
    ...
    function_name, function_params, function_param_names,
...
class CodeGenerator:
    ...
    def _generate_inner(self, unit, top_decls) -> str:
        ...
        self._gnu_inline_funcs: dict[str, ast.FunctionDef] = {}
        for d in top_decls:
            if not (isinstance(d, ast.FunctionDef) and d.body is not None):
                continue
        ...
        functions = [
            ...
            if isinstance(d, ast.FunctionDef) and d.body is not None
    ...
    def _const_eval(self, expr: ast.Expression, name: str) -> int:
        ...
        if (
            ...
            if (
                expr.func.name.text == "__builtin_choose_expr"
                and len(expr.args) == 3
            ):
            ...
            if (
                expr.func.name.text == "__builtin_constant_p"
                and len(expr.args) == 1
            ):
                try:
    ...
    def _function(self, fn: ast.FunctionDef) -> list[str]:
        ctx = _FuncCtx()
        ...
        # _function pre-pass.
        nested_decls: list[ast.FunctionDef] = []
        def _collect_nested_decls(node):
            if node is None:
                return
            if isinstance(node, ast.FunctionDef) and node.body is not None:
    ...
    def _call_inline_gnu(
        ...
        if len(items) == 1 and isinstance(items[0], ast.ReturnStmtValue):
            ...
            if not isinstance(then_branch, (ast.ReturnStmt, ast.ReturnStmtValue)):
                raise CodegenError(
    ...
    def _type_of(self, expr: ast.Expression, ctx: _FuncCtx) -> _ltypes.TypeNode:
        ...
        if isinstance(expr, (ast.Call, ast.CallNoArgs)):
            if isinstance(expr.func, ast.Identifier):
                ...
                if (
                    expr.func.name.text == "__builtin_shuffle"
                    and len(expr.args) >= 1
                ):
    ...
    def _eval_expr_to_edx_eax(
        ...
        if isinstance(expr, (ast.Call, ast.CallNoArgs)):
            ...
            if (
                ...
                and expr.func.name.text in ("llabs", "__builtin_llabs")
                and len(expr.args) == 1
            ):
    ...
    def _address_of(self, expr: ast.UnaryOp, ctx: _FuncCtx) -> list[str]:
        ...
        if isinstance(expr.operand, ast.Identifier):
            return self._identifier_address(expr.operand.name.text, ctx)
        if isinstance(expr.operand, ast.Index):
    ...
    def _inc_dec_bitfield_ll(
        ...
        Address-once: compute &storage_unit and the current bit-field
        value into hidden slots, then RMW [addr_slot] with new value
        +/- 1. Returns EDX:EAX as the (pre or post) value per C
        semantics. Without the snapshot, the previous implementation
        evaluated `expr.operand` up to three times — once via
        `_bitfield_load_ll` and twice via `_bitfield_store_ll_simple`
        (which itself called `_member_address` for the store address
        ...
        clear_low = (~positioned_mask) & 0xFFFFFFFF
        clear_high = ((~positioned_mask) >> 32) & 0xFFFFFFFF
        # Hidden slots: address (4 B) and current value (8 B).
        addr_slot_name = f"__incdec_bf_addr_{id(expr)}"
        ...
        if bit_width <= 32:
            ...
            out.append(f"        and     edx, {high_m}")
        # Sign-extend if signed and width < 64.
        if not is_unsigned and bit_width < 64:
            if bit_width <= 32:
                shift = 32 - bit_width
                if shift:
                ...
                out.append(f"        shl     edx, {shift}")
                out.append(f"        sar     edx, {shift}")
        # Save snapshot (= old value).
        out.append(f"        mov     {_ebp_addr(snap_disp)}, eax")
        out.append(f"        mov     {_ebp_addr(snap_disp + 4)}, edx")
        # 3. Compute `snap + 1` (or - 1) into EDX:EAX.
        if op_text == "+":
            out.append("        add     eax, 1")
            out.append("        adc     edx, 0")
        else:
            out.append("        sub     eax, 1")
            out.append("        sbb     edx, 0")
        # 4. Mask + position the new value, RMW [addr_slot].
        if low_m != 0xFFFFFFFF:
            out.append(f"        and     eax, {low_m}")
        if bit_width <= 32:
            out.append("        xor     edx, edx")
        ...
        out.append("        pop     edi")
        out.append("        pop     esi")
        # 5. Yield the right value: prefix → new (re-extract from
        #    storage so sign-ext semantics match _bitfield_load_ll);
        #    postfix → old (snapshot we already have).
        if expr.is_prefix:
            out += self._bitfield_load_ll_via_addr(addr_disp, bf)
        else:
            out.append(f"        mov     eax, {_ebp_addr(snap_disp)}")
            out.append(f"        mov     edx, {_ebp_addr(snap_disp + 4)}")
        return out
    ...
    def _bitfield_load_ll_via_addr(
        ...
        the bit-field value sign- or zero-extended to 64 bits.
        """
        bit_offset, bit_width, member_ty, _ = bf
        is_unsigned = self._is_unsigned(member_ty)
        mask = (1 << bit_width) - 1
