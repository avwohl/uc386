...
| 970217-1 | C99 VMT (sub-feature) | `sub(int i, int array[i++])` — size expr in a variably-modified array parameter must be evaluated on entry for its `i++` side effect (i→11). `_sub` never emits the `i++`. Distinct from dealloc; param `size_side_effects` capture exists but isn't wired for this declarator shape. |
| 20040411-1 | C99 VLA-typedef `sizeof` | `typedef int c[i+2]; sizeof(c)`. `_emit_runtime_size_of` handles VLA `ArrayType`s and direct-VLA-var `sizeof` already works (runtime `n*elem`), but `sizeof(<typedef-name>)` passes an **unresolved** `TypeName(TypedefNameSpec('c'))` so `_type_has_vla` is False → static `mov eax,4`. Needs: resolve the typedef to its underlying VLA `ArrayType` in the sizeof path (cf. the `_cast` typedef fix `34d45ab`) **plus** C99 6.7.7p2 evaluate-once-at-typedef-declaration capture (the size is fixed when the typedef is reached, not re-evaluated per `sizeof`). |
| 20041218-2 | C99 VMT-in-struct `sizeof` | `struct s{char b[n];}; sizeof(struct s)` — runtime struct size with a variably-modified member. `_emit_runtime_size_of` has a StructType branch; gap is capturing the member dim at the struct-decl point. |
