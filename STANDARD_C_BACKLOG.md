...
| Test(s) | Class | Note |
|---|---|---|
| 970217-1 | C99 VMT | `sub(int i, int array[i++])` — size expr in a variably-modified array parameter must be evaluated on entry for its side effect (i→11). Part of the VLA/VMT feature, not a corner miscompile (`_sub` simply never emits the `i++`). |
| pr43220, vla-dealloc-1, 20040811-1, 20040411-1, 20041218-2 | C99 VLA | VLA / variably-modified types — one scoped feature (frame alloca + dealloc on scope exit), multi-site. |
