...
**Skeleton + triage + multi-TU compile clean** under
`addons/gnu/micropython/`. A runnable `micropython.bin` is still
work-in-progress, but the prerequisites are now confirmed:
...
- **Multi-TU compile** of the **upstream minimal port**
  ...
  9 `shared/`, 143 sources in one uc386 invocation): produces a
  **1.5 MB .asm with zero compile errors** and only **3
  unresolved externs** — `mp_frozen_mpy_content`,
  `mp_frozen_names`, `mp_qstr_frozen_const_pool`, all normally
  generated by `tools/mpy-tool.py` from a frozen-modules manifest.
  The minimal port's mpconfigport.h forces `MICROPY_MODULE_FROZEN_MPY = 1`;
  `addons/gnu/micropython/uc386-dos/mpconfigport.h` flips that to
  `0` so a port-shim build resolves cleanly without a frozen-MPY
  generator.
