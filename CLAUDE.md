...
- Expressions: integer literals, identifier reads, unary `+ - ~ ! ++ -- & *`, binary `+ - * / % & | ^ << >> == != < > <= >= && ||`, assignment `=` to either an identifier or `*p`, compound assignment (Identifier lvalue only), and ternary `?:`. No casts, no pointer arithmetic with size scaling, no arrays yet.
