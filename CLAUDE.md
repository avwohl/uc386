...
- Expressions: integer literals, identifier reads, unary `+ - ~ ! ++ --` (prefix and postfix), binary `+ - * / % & | ^ << >> == != < > <= >= && ||`, assignment `=`, compound assignment `+= -= *= /= %= &= |= ^= <<= >>=`, and ternary `?:`. Lvalues must be identifiers (params or locals). No casts or pointer/array ops yet.
